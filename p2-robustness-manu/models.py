...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, 
        ...
        ##########################################################
        # YOUR CODE HERE
        samples = self._sample_noise_predictions(inputs, n0, batch_size)
        top_class = samples.argmax()
        noise_samples = self._sample_noise_predictions(inputs, num_samples, batch_size)
        p_A_lower_bound = lower_confidence_bound(noise_samples[top_class], num_samples, alpha)
        ##########################################################
        ...
        if p_A_lower_bound < 0.5:
            ...
            ##########################################################
            # YOUR CODE HERE
            radius = norm.ppf(p_A_lower_bound) * self.sigma
            ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, 
        ...
        ##########################################################
        # YOUR CODE HERE
        winning_counts = torch.topk(class_counts, 2)
        count1, count2 = winning_counts.values
        winning_class = winning_counts.indices[0]
        test_result = binom_test(count1, count1 + count2, p=0.5)
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, 
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                one_hot = nn.functional.one_hot(predictions)
